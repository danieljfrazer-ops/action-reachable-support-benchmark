# Stage 0A contract v3.8 (consolidated, normative)

**Status:** version 3.8, 7 September 2026, after round 5 (safe corrections; D-11 pending). **Supersedes** `stage-0a-contract-v3.7.md`, `stage-0a-contract-v3.6.md`, `stage-0a-contract-v3.5.md`, `stage-0a-contract-v3.4.md`, `stage-0a-contract-v3.3.md`, `stage-0a-contract-v3.2.md`, `stage-0a-contract-v3.1.md`, `stage-0a-contract-v3.md`, `stage-0a-contract-draft.md`, `stage-0a-contract-v2.md`, `stage-0a-contract-v2.1.md`, `stage-0a-contract-v2.2.md`, and the contract-related patches in `gemini-v3.1/`, `gemini-v3.2/`, `gemini-v3.3/`. Incorporates Codex v3.4 findings L1, L4, L5, L9, L12, L13. Sections are marked **[N]** normative (implement exactly) or **[I]** informative.
...
| ARL_0 | target average run length to false alarm, defined as the **fresh-start run length to first alarm** (D-6b; what a single-event episode experiences) | 1000 steps. **Calibration rule (D-2a):** on the calibration split, per (environment, regime, confounder, **distractor_level, delay**, estimator) cell (R3-7, CX-06; the threshold depends on channel count and delay), shared across seeds, continue until ≥ 400 false-alarm run lengths are observed and the 95 percent interval for the mean run length lies inside [900, 1100]; hard cap 2,000,000 steps. A method that cannot attain the band is **never excluded**: it is reported under the predeclared partial-order endpoint (its best attainable (ARL, HPDT) pair) and the primary is not recomputed without it |
...
| generator | reference implementation and frozen configuration set | `executable-proofs/gate/reference_generator.py`; configuration seeds {0..9}; perturbation set: coupling × {0.5, 1, 2}, noise × {0.5, 1, 2} (D-10.1) |
...
## A0. Reference generator [N] (D-10.1)

All instances used in simulations, pilots and confirmation are drawn by `reference_generator.py` from the frozen configuration family: A_b spectral radius 0.85 with declared sparsity; B dense with |entries| ≥ c_min; C_d; A_d, A_w, A_x diagonal AR(1) at 0.9; policy W_o supported on body channels only with declared gain; W_u scaled to meet ρ_min; G on the confounded half of x; four padding channels; **no copies** in confirmatory instances (D-10.6); zero initial state with burn-in; noise keyed by (seed, variable, t). Each instance is certified (closed-loop radius on the augmented state, saturation, ρ_min witness, operational-effect intervals, faithfulness bound |entry| ≥ c_min on A_b and B, and **CL-4: the confirmatory event removes at least one observed channel from S^obs,ε**) or resampled. Noise streams are keyed by a fixed integer namespace, never by a Python string hash. Burn-in of 2,000 steps precedes every episode from a separately keyed prefix. Configuration seeds 0 to 9 are development instances; the confirmation instance set is disjoint, hashed, and held by Daniel until specs and constants are frozen (D-11.3). Three independent implementations of either arm on the same configuration seed must agree to Monte Carlo error; this is the exit condition for design review, checked on the configuration set and the perturbation set.
...
E6 **Independent acceptance** (L16): a black-box acceptance suite written from `interface-spec-v4.md` (a one-page frozen interface), not from the implementation; plus one hand-computed rectangular, noisy case exercising every outcome class (detected, missed, terminated) that Daniel checks against both the pipeline and `contract_ref.py` (OP-31).

## F. Estimator interface [N]

Per step: p_c ∈ [0,1] per channel (probability c ∈ S^obs,ε; from a declared probabilistic model or a calibrator fitted on a separate split; logged); α^S_t ∈ {0,1}. Optional, scored only in later phases or appendices: α^R_t, R̂, assignment distributions. **Paper 1 scores p_c and α^S only.** Every baseline declares its **information set** (L10): which channels, whether oracle coordinates or P are visible, whether probe data may train the controller. The estimator receives full transitions (`interface-spec-v4.md`), including the applied action and probe marker (CX-03). **Misspecification registry** (CX-18): R1 transforms are frozen and hashed: family L drift = parameters of the residual model fitted on pre-drift data then held while A_b, B_t are perturbed by +10 percent on the diagonal; family N = linear residual model fitted on family N data; T2 = linear autoregressive residual model. R0 on T2 (CL-5, GM-5) = an MLP residual model trained to convergence on 200,000 fault-free in-distribution steps with a Ljung-Box whiteness test on innovations at the pilot.
...
- **Primary outcome (D-9.2, D-9.5 tier 1).** **Online support ranking:** threshold-free AUC of the estimator's raw per-channel support statistic a_c against S^obs,ε at offsets_rank, primary offset 500, scored per episode across observation channels and paired by seed and cell; invariant to any monotone calibrator, so it uses no supervised data. Co-reported: AUPRC per episode with prevalence and lift over prevalence, and a grouped AUPRC collapsing sensor copies to one equivalence class and padding to one declared negative class (sensitivity, not a gate). Cells are never pooled.
...
- **Decision rules (D-9.3, D-10.3).** Let Δ_AUC = AUC(seq-IBD) − AUC(passive comparator) per episode, paired within instance and cell, intervals clustered by instance. **Confounding benefit** = [Δ_AUC(present) − Δ_AUC(absent)] within regime: superiority if the lower 95 percent bound > δ_AUC on both SCM families and the sign holds on both control-tier environments; futility if the upper bound < δ_AUC on either SCM family; inconclusive otherwise. **R0-present** is a positive control: lower bound of Δ_AUC(present) > δ_AUC expected; failure is an anomaly that stops the phase. **R0-absent** is a comparator-competence validity check (D-10.3): in every required (environment, distractor_level) cell the lower 95 percent bound of the comparator's AUC without confounding must be ≥ 0.85; Δ_AUC(absent) is reported. If the floor fails in any cell, the comparator specification, not the rule, is defective and the phase stops. The double-difference interaction is descriptive. Prevalence-adjusted AUPRC repeats each rule as a sensitivity, not a gate. The pilot's power arm sizes seeds for this conjunction (v4.4 H2 applies to these rules).
...
- **Alarm matching (OP-12, R3-9):** two stages in `contract_ref.py`, both tested: `count_alarms(raw, p, r) -> counted alarm times` (p consecutive raises; refractory r) and `match_alarms(alarm_times, event_times, H_det, episode_end) -> (delays, outcomes, false_alarms)`; an alarm at delay d ∈ (0, H_det] after event e and before the next event is attributed to e.
...
`interface-spec-v4.md` (frozen, one page); package with declarative SCM, generator, separate oracle package, gate tests and mutants, baselines, ledger with manifests; `coverage-matrix.md`; runtime pilot report. Package name: **to be chosen after a collision check** (L17); `boundary-bench` is taken.
