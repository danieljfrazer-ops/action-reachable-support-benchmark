# Interface spec v4 (frozen, one page) — basis for the independent black-box acceptance suite (contract E6)

Supersedes `interface-spec-v3.md` (round 4: R4-CX-03/05, R4-17/18, GM-09).
...
- `fit_predictor(transitions)`: consumes the non-probed fault-free fit split (predictor-type arms; no-op for others). `fit_alarm_reference(transitions)`: consumes the arm-specific fault-free stream (probed for the probed arm) to set the alarm reference (k, ā, v). Two typed calls replace the single `calibrate` (R4-CX-05). `set_threshold(h)`: alarm threshold from the harness's ARL_0 calibration.
- `update(transition) -> (raw_support[C], p[C] in [0,1], stat: float, raise: {0,1})`: `raw_support` is the arm's raw per-channel statistic (a_c or q_c) on which the primary AUC is computed by the evaluator; it is returned every step (piecewise constant between epochs allowed) (R4-CX-03)., once per environment step (a probe step is an environment step whose action was replaced); outputs may be piecewise constant between epochs (GM3-9). `raise = 1 iff stat > h` with **no persistence applied**; the harness owns persistence p (in the estimator's declared unit, steps or epochs) and refractory r via `contract_ref.count_alarms` (R3-10). Declared flag `statistic_is_monotone_in_alarm=True`; declared `persistence_unit`; declared `sign_blindness` = `formula` (the probed comparator's frozen formula contains no randomisation-stratified statistic; enforced by source hash and metamorphic fixtures over non-probe unit actions, not by withholding data, R4-CX-10).
