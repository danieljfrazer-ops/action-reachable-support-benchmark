# Stage 0A contract v3.2 (consolidated, normative)

**Status:** version 3.2, 6 September 2026, after round 2 and decisions D-1 to D-5. **Supersedes** `stage-0a-contract-v3.1.md`, `stage-0a-contract-v3.md`, `stage-0a-contract-draft.md`, `stage-0a-contract-v2.md`, `stage-0a-contract-v2.1.md`, `stage-0a-contract-v2.2.md`, and the contract-related patches in `gemini-v3.1/`, `gemini-v3.2/`, `gemini-v3.3/`. Incorporates Codex v3.4 findings L1, L4, L5, L9, L12, L13. Sections are marked **[N]** normative (implement exactly) or **[I]** informative.
...
| episode_len, event_t | confirmation schedule (F7) | 2,000; 1,000 [provisional; see D-6] |
...
**Comparator identity (D-3a).** The classical comparator is **CUSUM on innovations of a channel-agnostic linear predictor in every cell**. R0 = the linear predictor fitted on in-distribution fault-free data (correct only on family L; on family N and T2 it is the best linear model). R1 = the same predictor class under the frozen misspecification transform. The words "correct model" are dropped everywhere. Sequential IBD is specified in `sequential-ibd-spec.md` (D-4a), normative once signed by Daniel. Every estimator's `update` returns a **continuous statistic** alongside the binary alarm so the harness can calibrate a threshold (OP-4; interface v3).
