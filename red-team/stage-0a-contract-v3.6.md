# Stage 0A contract v3.6 (consolidated, normative)

**Status:** version 3.6, 7 September 2026, after round 4 (safe corrections; D-10 pending). **Supersedes** `stage-0a-contract-v3.5.md`, `stage-0a-contract-v3.4.md`, `stage-0a-contract-v3.3.md`, `stage-0a-contract-v3.2.md`, `stage-0a-contract-v3.1.md`, `stage-0a-contract-v3.md`, `stage-0a-contract-draft.md`, `stage-0a-contract-v2.md`, `stage-0a-contract-v2.1.md`, `stage-0a-contract-v2.2.md`, and the contract-related patches in `gemini-v3.1/`, `gemini-v3.2/`, `gemini-v3.3/`. Incorporates Codex v3.4 findings L1, L4, L5, L9, L12, L13. Sections are marked **[N]** normative (implement exactly) or **[I]** informative.
...
| n_min_sign | minimum probe observations per sign group in the sign-randomised statistic; below it the cell contributes zero | 3 |
...
- **Decision rules (D-9.3).** Let Δ_AUC = AUC(seq-IBD) − AUC(passive comparator) per seed and cell. **Confounding benefit** = [Δ_AUC(present) − Δ_AUC(absent)] within regime: superiority if the lower 95 percent bound > δ_AUC on both SCM families and the sign holds on both control-tier environments; futility if the upper bound < δ_AUC on either SCM family; inconclusive otherwise. **R0-present** is a positive control: lower bound of Δ_AUC(present) > δ_AUC expected; failure is an anomaly that stops the phase. **R0-absent** is an equivalence cell: two one-sided tests, 90 percent interval for Δ_AUC(absent) inside (−δ0_AUC, +δ0_AUC). The double-difference interaction is descriptive. Prevalence-adjusted AUPRC repeats each rule as a sensitivity, not a gate. The pilot's power arm sizes seeds for this conjunction (v4.4 H2 applies to these rules).
