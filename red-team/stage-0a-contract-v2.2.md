# Stage 0A contract v2.2: executable structural-causal environment specification

Status: version 2.2, 6 September 2026. Supersedes v2.1 after the second Gemini audit (H1, H4). Patched sections are marked [v2.1] or [v2.2]. Every equation in this contract ships with an executable check in `executable-proofs/`; the building agent must run them before scaffolding.
...
**Coupling note [v2.2] (H1).** In a body with internal coupling (off-diagonal A_b, or inertial coupling in a physical simulator), losing an actuator changes R but need not remove any component from S at horizons ≥ 2, because the lost joint remains reachable through the others. Verified in `executable-proofs/gemini_v32_checks.py`. Consequently: (i) the "actuator loss" row above reads "changed iff reachability changes" and the oracle decides; (ii) Tier T2 environments in Paper 1 are restricted to single-actuator systems (CartPole, inverted pendulum) where loss of the actuator empties the support unambiguously; coupled multi-joint simulators are deferred to Paper 2 and their actuator-loss events are classified as R-changes.
...
Per step, every estimator returns [v2.2]:
...
2. Typed support-change alarm α^S_t ∈ {0,1}.
3. Optional, evaluated only in phases with structured estimators: α^R_t, α^P_t; R̂_{t,h}(a) over 𝒜 × ℋ; per-channel assignment distributions.

**Paper 1 scoring [v2.2] (G6, H4).** Paper 1 baselines are scored **exclusively** on p_c (support S^obs,ε) and α^S (support-change alarm). R and P remain ground-truth objects in the contract for generator and oracle verification and for the mutant tests, and are scored only for methods that natively estimate them, in later phases. Estimators without a native mechanism for R or P emit nothing for them, and no comparison on R or P is made in Paper 1. A constant output has no operating curve and would produce a fictitious comparison.
